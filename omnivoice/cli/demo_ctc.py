...
# --- Subtitle Generation (ctc-forced-aligner ONNX) ---
...
def text_to_srt_ctc(text: str, audio_tuple, language="zh") -> str:
    ...
    with tempfile.NamedTemporaryFile(suffix=".txt", delete=False, mode="w", encoding="utf-8") as f_text:
        f_text.write("\n".join(segments))
    ...
    try:
        aligner = ctc_forced_aligner.AlignmentSingleton()
        # ctc-forced-aligner uses 'cmn' for Chinese
        lang_code = "cmn" if language in ["zh", "cmn", "chinese"] else (language[:3] if language else "eng")
        success = aligner.generate_srt(audio_path, text_path, srt_path, language=lang_code)
        if success:
            with open(srt_path, "r", encoding="utf-8") as f:
                return f.read()
        return "Failed to generate alignment."
    except Exception as e:
        return f"Alignment error: {str(e)}"
    finally:
        for p in [audio_path, text_path, srt_path]:
...
def build_demo(model: OmniVoice):
    ...
    def _gen_core(text, language, ref_audio, instruct, num_step, guidance_scale, denoise, speed, duration, preprocess_prompt, postprocess_output, mode, generate_srt=True, ref_text=None):
        if not text or not text.strip(): return None, "", None, "Please enter text."
        ...
        if mode == "clone":
            if not ref_audio: return None, "", None, "Need ref audio."
        ...
        if generate_srt:
            srt_content = text_to_srt_ctc(text, audio_tuple, language=lang)
    ...
    with gr.Blocks(title="OmniVoice (CTC-ONNX)") as demo:
        gr.Markdown("# OmniVoice - CTC-ONNX Alignment")
        with gr.Tabs():
